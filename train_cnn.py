...
def main():
    ...
    print("Number of float-valued parameters:", count_parameters(model)) # output number of learnable parameters in the model
    ...
    model, start_epoch, stats = restore_checkpoint(model, config("target.checkpoint")) # restore model from checkpoint if exists
